...
import { startJobHeartbeat, touchRunningJob } from "@/lib/job-heartbeat.server";
  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        const stopHeartbeat = kaggleJobId ? startJobHeartbeat(supa, kaggleJobId) : () => {};
        ...
        try {
          ...
          const result = streamText({
            ...
            onStepFinish: async (step) => {
              ...
              if (isKaggle && kaggleJobId) {
                await touchRunningJob(supa, kaggleJobId).catch((error) => {
          ...
          if (!responseBody) {
            ...
            void consumeStream({ stream: result.toUIMessageStream() }).finally(() => {
              clearWatchdog();
              stopHeartbeat();
            });
          ...
          // Do not bind server-side consumption to the browser's abort signal.
          const serverConsumption = consumeStream({ stream: toServer }).finally(() => {
            clearWatchdog();
            stopHeartbeat();
          });
          ...
          try {
            ...
            event.waitUntil(serverConsumption);
          } catch {
            // Fallback if not in H3Event context (e.g., during testing or non-TanStack runtimes)
            // Consumption is already running; never try to acquire a second reader.
            void serverConsumption;
          }
          ...
          if (kaggleJobId) {
            await supa.from("coding_jobs").update({
              ...
              finished_at: new Date().toISOString(),
            }).eq("id", kaggleJobId).eq("status", "running");
          }
          throw error;
        }
      },
