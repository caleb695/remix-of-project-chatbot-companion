      ...
      POST: async ({ request }) => {
        ...
        const result = streamText({
          ...
          prepareStep: ({ steps, stepNumber }) => {
            ...
            // CRITICAL: prevent the "done immediately" symptom. The agent loop
            // in the AI SDK only continues a step when it produced tool calls;
            // a text-only first step ends the whole run right there. In build /
            // debug / improve modes we must force the model to interact with its
            // tools first (read the notebook/files, then edit), otherwise it can
            // "finish" with a plan/greeting and no work done. Plan mode keeps
            // auto because a written plan IS the valid output.
            if (stepNumber === 0 && mode !== "plan") {
