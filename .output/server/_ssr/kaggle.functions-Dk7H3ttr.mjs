...
import { t as getServerFnById } from "../__23tanstack-start-server-fn-resolver-BGTm-dkK.mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/kaggle.functions-Dk7H3ttr.js
...
var addRepoSelection = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((input) => objectType({
...
createServerFn({ method: "GET" }).middleware([requireSupabaseAuth]).inputValidator((input) => objectType({ id: stringType().uuid() }).parse(input)).handler(createSsrRpc("efa440b4311e373bbe3f4bb8a8f7d9e763f339a8e716a7937d7ab2c1a2cb7c32"));
var removeRepoSelection = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((input) => objectType({ id: stringType().uuid() }).parse(input)).handler(createSsrRpc("5c46a8c7f4107f0d11228ba99265b67c97b569009ab856968686e41bc4483633"));
createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((input) => objectType({ repoId: stringType().uuid() }).parse(input)).handler(createSsrRpc("a0196570d479357d50e8b01c9cad32872c32f205b29dd876fabf47ca17239a31"));
var commitAndPush = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((input) => objectType({
...
createServerFn({ method: "GET" }).middleware([requireSupabaseAuth]).inputValidator((input) => objectType({ repoId: stringType().uuid() }).parse(input)).handler(createSsrRpc("e112093d4fe65c6dcb569054830b78ff48691e7b06ef4607f563fcd0ba9412ea"));
createServerFn({ method: "GET" }).middleware([requireSupabaseAuth]).inputValidator((input) => objectType({
	repoId: stringType().uuid(),
	path: stringType()
...
var addKaggleNotebook = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((i) => objectType({ ref: stringType().min(3).max(200) }).parse(i)).handler(createSsrRpc("b832d657aad7abc4d28fa766d262862dbc028c774a000ecd53bd6e65673d9aff"));
var removeKaggleNotebook = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((i) => objectType({ id: stringType().uuid() }).parse(i)).handler(createSsrRpc("31731067b94acfbcbf32bc1df0f96c20df6da7e9bd13f83cca0019a5c48eadff"));
/** Re-pull from Kaggle, discarding staged edits. */
var syncKaggleNotebook = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((i) => objectType({ id: stringType().uuid() }).parse(i)).handler(createSsrRpc("760b9e27f6a7737e5c912b41d99e75b01a2679c82764c912ff66a7ed870314ca"));
var getKaggleStaged = createServerFn({ method: "GET" }).middleware([requireSupabaseAuth]).inputValidator((i) => objectType({ id: stringType().uuid() }).parse(i)).handler(createSsrRpc("c7061e41b1a568ea9a2449b8be28780c46c956a193b0f234fc7c3c8319a8e5fb"));
/** Push the staged notebook source back to Kaggle (a new version). */
var pushKaggleNotebook = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((i) => objectType({ id: stringType().uuid() }).parse(i)).handler(createSsrRpc("b92dee861c9f13b4c7dec5cb60312a1dfc581c74745b6f497aa62a9c161120db"));
//#endregion
