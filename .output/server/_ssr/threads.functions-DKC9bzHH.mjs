...
//#region node_modules/.nitro/vite/services/ssr/assets/threads.functions-DKC9bzHH.js
...
var deleteThread = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).validator((input) => objectType({ id: stringType().uuid() }).parse(input)).handler(createSsrRpc("568557aff3b98c509b2e18e3b33bc94e0f42eaf3bf5efa80b0f9c8e64ed87602"));
var getThreadMessages = createServerFn({ method: "GET" }).middleware([requireSupabaseAuth]).validator((input) => objectType({ threadId: stringType().uuid() }).parse(input)).handler(createSsrRpc("4de48302a58b8017a3864af8f5d4bfff1891bb7bbd691f1f10fb1c616272eab8"));
...
var getThread = createServerFn({ method: "GET" }).middleware([requireSupabaseAuth]).validator((input) => objectType({ id: stringType().uuid() }).parse(input)).handler(createSsrRpc("d3b5c99710bd28fef85ba35dd8e9416271f29ded53625108faccb3089c6e0da5"));
