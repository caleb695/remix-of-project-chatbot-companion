...
//#region node_modules/.nitro/vite/services/ssr/assets/threads.functions-D9vjPUdB.js
...
var createThread = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).validator((input) => objectType({
...
var deleteThread = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).validator((input) => objectType({ id: stringType().uuid() }).parse(input)).handler(deleteThread_createServerFn_handler, async ({ context, data }) => {
...
var getThreadMessages = createServerFn({ method: "GET" }).middleware([requireSupabaseAuth]).validator((input) => objectType({ threadId: stringType().uuid() }).parse(input)).handler(getThreadMessages_createServerFn_handler, async ({ context, data }) => {
...
var renameThread = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).validator((input) => objectType({
	id: stringType().uuid(),
...
var getThread = createServerFn({ method: "GET" }).middleware([requireSupabaseAuth]).validator((input) => objectType({ id: stringType().uuid() }).parse(input)).handler(getThread_createServerFn_handler, async ({ context, data }) => {
...
var updateThread = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).validator((input) => objectType({
	id: stringType().uuid(),
	model: stringType().optional(),
