...
//#region node_modules/.nitro/vite/services/ssr/assets/attachments.functions-DzchDW1H.js
...
var listAttachments = createServerFn({ method: "GET" }).middleware([requireSupabaseAuth]).inputValidator((i) => objectType({ threadId: stringType().uuid() }).parse(i)).handler(listAttachments_createServerFn_handler, async ({ context, data }) => {
...
var deleteAttachment = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((i) => objectType({ id: stringType().uuid() }).parse(i)).handler(deleteAttachment_createServerFn_handler, async ({ context, data }) => {
