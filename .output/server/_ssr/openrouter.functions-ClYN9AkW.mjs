...
//#region node_modules/.nitro/vite/services/ssr/assets/openrouter.functions-ClYN9AkW.js
...
*/
var installCoderWorkflow = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((i) => objectType({ repoId: stringType().uuid() }).parse(i)).handler(createSsrRpc("383c2b9874e19cea47ee663539bd20129142921efe99c79ed3fd099b06632900"));
...
var getJob = createServerFn({ method: "GET" }).middleware([requireSupabaseAuth]).inputValidator((i) => objectType({ id: stringType().uuid() }).parse(i)).handler(createSsrRpc("432b934c493a65b20930541aa425464f2c52d0a8716794775211fc250ec5a8e5"));
/** The full patch the run produced, for the review screen. */
var getJobDiff = createServerFn({ method: "GET" }).middleware([requireSupabaseAuth]).inputValidator((i) => objectType({ id: stringType().uuid() }).parse(i)).handler(createSsrRpc("ff56f97c1602fd8f21d00892d160b37595d659ba851a485a5c9265ce78de51cb"));
/** Merge the run's review branch into the working branch — the user's approval. */
var approveJob = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((i) => objectType({ id: stringType().uuid() }).parse(i)).handler(createSsrRpc("1a3b9034e7640601f7d2fd280fa3f78f56ec077718405469a799410b4a1f2d8a"));
/** Throw the run's changes away without touching the working branch. */
var discardJob = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((i) => objectType({ id: stringType().uuid() }).parse(i)).handler(createSsrRpc("84414820f38145d46d0817d5ecdd9567dfcef88237251d9b3498becd3535eefb"));
var listJobsForThread = createServerFn({ method: "GET" }).middleware([requireSupabaseAuth]).inputValidator((i) => objectType({ threadId: stringType().uuid() }).parse(i)).handler(createSsrRpc("ae88e471ba0555f9a707a0c7ca719d83edb88cc0e440cc622590141b52c8d806"));
var cancelJob = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((i) => objectType({ id: stringType().uuid() }).parse(i)).handler(createSsrRpc("9e68af82cd2a1c15199838803e19d0ecc2492b63dfb855608f5f90105b675297"));
...
var getLatestIndexJob = createServerFn({ method: "GET" }).middleware([requireSupabaseAuth]).inputValidator((i) => objectType({ repoId: stringType().uuid() }).parse(i)).handler(createSsrRpc("cb2d0481864d2f68e1c998e8688fc35ed55f354828ad77387cad45d70248a1d1"));
