...
//#region node_modules/.nitro/vite/services/ssr/assets/github.functions-FYCCfkft.js
...
var getRepoSelection = createServerFn({ method: "GET" }).middleware([requireSupabaseAuth]).inputValidator((input) => objectType({ id: stringType().uuid() }).parse(input)).handler(getRepoSelection_createServerFn_handler, async ({ context, data }) => {
...
var removeRepoSelection = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((input) => objectType({ id: stringType().uuid() }).parse(input)).handler(removeRepoSelection_createServerFn_handler, async ({ context, data }) => {
...
var syncRepoFromGithub = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((input) => objectType({ repoId: stringType().uuid() }).parse(input)).handler(syncRepoFromGithub_createServerFn_handler, async ({ context, data }) => {
...
var listWorkingFiles = createServerFn({ method: "GET" }).middleware([requireSupabaseAuth]).inputValidator((input) => objectType({ repoId: stringType().uuid() }).parse(input)).handler(listWorkingFiles_createServerFn_handler, async ({ context, data }) => {
...
var getWorkingFile = createServerFn({ method: "GET" }).middleware([requireSupabaseAuth]).inputValidator((input) => objectType({
	repoId: stringType().uuid(),
	path: stringType()
