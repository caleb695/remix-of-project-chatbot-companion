...
//#region node_modules/.nitro/vite/services/ssr/assets/agent.functions-nUlq9Ncr.js
...
var getLatestTask = createServerFn({ method: "GET" }).middleware([requireSupabaseAuth]).inputValidator((i) => objectType({ threadId: stringType().uuid() }).parse(i)).handler(getLatestTask_createServerFn_handler, async ({ context, data }) => {
...
var getStagedChanges = createServerFn({ method: "GET" }).middleware([requireSupabaseAuth]).inputValidator((i) => objectType({ repoId: stringType().uuid() }).parse(i)).handler(getStagedChanges_createServerFn_handler, async ({ context, data }) => {
...
var branchThread = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((i) => objectType({ threadId: stringType().uuid() }).parse(i)).handler(branchThread_createServerFn_handler, async ({ context, data }) => {
...
var getThreadSeed = createServerFn({ method: "GET" }).middleware([requireSupabaseAuth]).inputValidator((i) => objectType({ id: stringType().uuid() }).parse(i)).handler(getThreadSeed_createServerFn_handler, async ({ context, data }) => {
