...
import { z } from "zod";
//#region src/lib/agent.functions.ts?tss-serverfn-split
...
var listAgentEvents = createServerFn({ method: "GET" }).middleware([requireSupabaseAuth]).inputValidator((i) => z.object({
...
var getLatestTask = createServerFn({ method: "GET" }).middleware([requireSupabaseAuth]).inputValidator((i) => z.object({ threadId: z.string().uuid() }).parse(i)).handler(getLatestTask_createServerFn_handler, async ({ context, data }) => {
...
var getStagedChanges = createServerFn({ method: "GET" }).middleware([requireSupabaseAuth]).inputValidator((i) => z.object({ repoId: z.string().uuid() }).parse(i)).handler(getStagedChanges_createServerFn_handler, async ({ context, data }) => {
...
var setThreadMode = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((i) => z.object({
	id: z.string().uuid(),
	mode: z.enum([
		"plan",
		"build",
...
var branchThread = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((i) => z.object({ threadId: z.string().uuid() }).parse(i)).handler(branchThread_createServerFn_handler, async ({ context, data }) => {
...
var getThreadSeed = createServerFn({ method: "GET" }).middleware([requireSupabaseAuth]).inputValidator((i) => z.object({ id: z.string().uuid() }).parse(i)).handler(getThreadSeed_createServerFn_handler, async ({ context, data }) => {
