...
import { z } from "zod";
//#region src/lib/subagents.functions.ts?tss-serverfn-split
...
var getSubAgents = createServerFn({ method: "GET" }).middleware([requireSupabaseAuth]).inputValidator((i) => z.object({ threadId: z.string().uuid() }).parse(i)).handler(getSubAgents_createServerFn_handler, async ({ context, data }) => {
...
var setSubAgents = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((i) => z.object({
	threadId: z.string().uuid(),
	subAgents: z.array(subAgentSchema).max(20)
