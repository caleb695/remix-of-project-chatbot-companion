...
import { z } from "zod";
//#region src/lib/attachments.functions.ts?tss-serverfn-split
...
var listAttachments = createServerFn({ method: "GET" }).middleware([requireSupabaseAuth]).validator((i) => z.object({ threadId: z.string().uuid() }).parse(i)).handler(listAttachments_createServerFn_handler, async ({ context, data }) => {
...
var registerAttachment = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).validator((i) => z.object({
...
var setAttachmentCodeOnly = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).validator((i) => z.object({
	id: z.string().uuid(),
	codeOnly: z.boolean()
...
var deleteAttachment = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).validator((i) => z.object({ id: z.string().uuid() }).parse(i)).handler(deleteAttachment_createServerFn_handler, async ({ context, data }) => {
